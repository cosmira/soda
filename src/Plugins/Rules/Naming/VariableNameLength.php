...
final readonly class VariableNameLength implements RuleChecker
{
    public function __construct(private int $min = 3, private int $max = 16) {}

    #[\Override]
    public function check(EvaluationContext $context): Collection
    {
        return (new NameLengthChecker)
            ->check($context->withConfig($this->config(NameLengthChecker::VARIABLE)));
    }
