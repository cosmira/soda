...
final class MultilineConstantPhpDoc extends Check
{
    /**
    ...
    public function checkFile(FileFacts $file): iterable
    {
        foreach ((new NodeFinder)->findInstanceOf($file->nodes, ClassLike::class) as $class) {
            foreach ($class->getConstants() as $member) {
     ...
     * @return iterable<Violation>
     */
    private function violations(FileFacts $file, ClassLike $class, ClassConst $member): iterable
    {
        ...
        foreach ($member->consts as $item) {
            ...
            yield new Violation(
                ...
                line: $member->getLine(),
                message: sprintf('%s constant::%s must have a multiline PHPDoc comment', ucfirst($visibility), $name),
            );
        }
    }
     ...
     */
    private function visibility(ClassConst $member): string
    {
    ...
    {
        return 'multiline_constant_phpdoc';
    }
}
