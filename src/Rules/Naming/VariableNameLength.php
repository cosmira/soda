...
final class VariableNameLength extends Check
{
    /**
    ...
    {
        foreach ((new NodeFinder)->findInstanceOf($file->nodes, Variable::class) as $node) {
            $parent = $node->getAttribute('parent');
            $isDynamicName = $parent instanceof Variable && $parent->name === $node;
            $isDeclaredName = is_string($node->name) && ! $isDynamicName;
            if ($isDeclaredName) {
                yield from $this->violationsForName($file->path, $node->name, $node->getLine());
            }
        }
    }
