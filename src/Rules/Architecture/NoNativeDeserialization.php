...
final class NoNativeDeserialization extends Check
{
    /**
    ...
    public function checkFile(FileFacts $file): iterable
    {
        $nodes = (new NodeFinder)->find($file->nodes, fn (Node $node): bool => $node instanceof Expr\FuncCall && $node->name instanceof Name
                && strtolower($node->name->toString()) === 'unserialize');
        ...
        foreach ($nodes as $node) {
            yield new Violation(
                ...
                message: 'Native deserialization hides object construction. Decode validated data into explicit types.',
            );
        }
    }
    ...
    {
        return 'no_native_deserialization';
    }
