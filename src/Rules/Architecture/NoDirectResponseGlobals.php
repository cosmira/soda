...
final class NoDirectResponseGlobals extends Check
{
    /**
    ...
    public function checkFile(FileFacts $file): iterable
    {
        $nodes = (new NodeFinder)->find($file->nodes, fn (Node $node): bool => $node instanceof Expr\FuncCall && $node->name instanceof Name
                && in_array(strtolower($node->name->toString()), ['header', 'setcookie', 'setrawcookie', 'session_start'], true));
        ...
        foreach ($nodes as $node) {
            yield new Violation(
                ...
                message: 'Direct response and session globals hide output effects. Return an explicit response or use a boundary adapter.',
            );
        }
    }
    ...
    {
        return 'no_direct_response_globals';
    }
