...
final class NoSubprocessExecution extends Check
{
    /**
    ...
    public function checkFile(FileFacts $file): iterable
    {
        $nodes = (new NodeFinder)->find($file->nodes, fn (Node $node): bool => $node instanceof Expr\FuncCall && $node->name instanceof Name
                && in_array(strtolower($node->name->toString()), ['shell_exec', 'exec', 'system', 'popen'], true));
        ...
        foreach ($nodes as $node) {
            yield new Violation(
                ...
                message: 'Subprocess execution hides dependencies and failure modes. Use an injected, typed gateway.',
            );
        }
    }
    ...
    {
        return 'no_subprocess_execution';
    }
