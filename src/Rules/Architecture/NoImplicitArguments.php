...
final class NoImplicitArguments extends Check
{
    /**
    ...
    public function checkFile(FileFacts $file): iterable
    {
        $nodes = (new NodeFinder)->find($file->nodes, fn (Node $node): bool => $node instanceof Expr\FuncCall && $node->name instanceof Name
                && in_array(strtolower($node->name->toString()), ['func_get_args', 'func_get_arg', 'func_num_args'], true));
        ...
        foreach ($nodes as $node) {
            yield new Violation(
                ...
                message: 'Implicit argument access hides the callable contract. Declare every input explicitly.',
            );
        }
    }
