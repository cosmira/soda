...
final class NoDirectFilesystemMutation extends Check
{
    /**
    ...
    public function checkFile(FileFacts $file): iterable
    {
        $nodes = (new NodeFinder)->find($file->nodes, fn (Node $node): bool => $node instanceof Expr\FuncCall && $node->name instanceof Name
                && in_array(strtolower($node->name->toString()), ['file_put_contents', 'unlink', 'rename', 'mkdir'], true));
        ...
        foreach ($nodes as $node) {
            yield new Violation(
                ...
                message: 'Direct filesystem mutation hides infrastructure effects. Inject an explicit storage boundary.',
            );
        }
    }
    ...
    {
        return 'no_direct_filesystem_mutation';
    }
