...
final class NoRuntimeTypeAliases extends Check
{
    /**
    ...
    public function checkFile(FileFacts $file): iterable
    {
        $nodes = (new NodeFinder)->find($file->nodes, fn (Node $node): bool => $node instanceof Expr\FuncCall && $node->name instanceof Name
                && strtolower($node->name->toString()) === 'class_alias');
        ...
        foreach ($nodes as $node) {
            yield new Violation(
                ...
                message: 'Runtime type aliases hide the real contract. Migrate callers or declare an explicit adapter.',
            );
        }
    }
    ...
    {
        return 'no_runtime_type_aliases';
    }
